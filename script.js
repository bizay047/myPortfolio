...
document.addEventListener("DOMContentLoaded", () => {    
    ...
    // --- 5. Smooth Scroll Reveal Animation (ULTIMATE FAILSAFE) ---
    const observerOptions = {
        root: null,
        rootMargin: "0px", // Safest option for all mobile browsers
    ...
    const observer = new IntersectionObserver((entries, observer) => {
        entries.forEach((entry) => {
            if (entry.isIntersecting) {
                // Adds a tiny 100ms delay to make it feel smoother on phones
                setTimeout(() => {
    ...
    // Watch all elements, and automatically reveal them if JavaScript is struggling
    const revealElements = document.querySelectorAll('.reveal');
    revealElements.forEach((el) => {
        // Fallback: If element is already at the very top of the screen on load, reveal it instantly
